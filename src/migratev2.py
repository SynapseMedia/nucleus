...
if __name__ == '__main__':
    ...
    if REFRESH_MOVIES or empty_tmp:
        ...
        for resource in resolvers.resources:
            logger.info(f"{Log.BOLD}Starting migrations from {resource}{DB_DATE_VERSION}{Log.ENDC}")
            migration_result = resource()  # Run __call__ method and get result from resource

            # TODO validate scheme here

            logger.info(f"{Log.OKGREEN}Migration Complete for {resource}{Log.ENDC}")
            logger.info(f"{Log.OKGREEN}Inserting entries in mongo{Log.ENDC}")

        # TODO Merge here all the data

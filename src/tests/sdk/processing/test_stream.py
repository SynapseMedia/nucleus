...
def test_hls_protocol(mock_local_file_path: Path):
    """Should be valid HLS setup"""
    custom_settings = {"-vcodec": "libx265"}
    stream_input = stream.input(mock_local_file_path, **custom_settings)
    streaming = stream_input.hls
    assert streaming._params == {  # type: ignore
        "-video_source": mock_local_file_path,
        "-vcodec": "libx265",
    }
    assert streaming._protocol == "hls"  # type: ignore


def test_dash_protocol(mock_local_file_path: Path):
    """Should be valid DASH setup"""

    stream_input = stream.input(mock_local_file_path)
    streaming = stream_input.dash
    assert streaming._params == {"-video_source": mock_local_file_path}  # type: ignore
    assert streaming._protocol == "dash"  # type: ignore


def test_invalid_protocol(mock_local_file_path: Path):
    """Should fail with invalid protocol"""
    with pytest.raises(AttributeError):
        stream_input = stream.input(mock_local_file_path)
        stream_input.invalid


def test_protocol_quality(mock_local_file_path: Path):

...
def test_resolve_root_for_raw():
    ...
    processing.media.RAW_PATH = custom_dir
    processing.media.PROD_PATH = "prod"  # just to be sure that this is not called
    result_dir = processing.resolve(directory, is_prod=False)
    # (Path expected for raw, if path exists = True)
    expected = ("%s/%s" % (processing.media.RAW_PATH, directory), True)

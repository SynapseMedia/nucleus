...
@pytest.fixture()
def mock_local_path():
    return Path('tests/_mock/files')


@pytest.fixture()
def mock_local_json_path():
    return Path('tests/_mock/files/index.json')


@pytest.fixture()
def mock_local_image_path():
    return Path('tests/_mock/files/spidy.png')


@pytest.fixture()
def mock_local_video_path():
    return Path('tests/_mock/files/video.mp4')
...
def rpc_api_add_request(**kwargs: Any):
    mock_link = 'http://localhost:5001/api/v0/add?pin=False&quieter=True&hash=blake2b-208&cid-version=1&wrap-with-directory=False'
